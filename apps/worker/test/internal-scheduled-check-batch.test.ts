...
describe('internal scheduled check-batch route', () => {
  ...
  it('skips per-batch fragment writes when the scheduler defers them to its consolidated write', async () => {
    const now = new Date('2026-04-15T05:18:20.000Z').valueOf();
    vi.spyOn(Date, 'now').mockReturnValue(now);
    vi.mocked(runExclusivePersistedMonitorBatch).mockResolvedValue({
      runtimeUpdates: [
    ...
    const res = await worker.fetch(
      new Request('http://internal/api/v1/internal/scheduled/check-batch', {
        ...
        headers: {
          ...
          'Content-Type': 'application/json; charset=utf-8',
          'X-Uptimer-Internal-Format': 'compact-v1',
          'X-Uptimer-Runtime-Fragments-Defer': '1',
        },
        body: JSON.stringify({
          token: 'test-admin-token',
          ids: [1],
    ...
    expect(res.status).toBe(200);
    // Runtime updates still come back so the scheduler can write them exactly once.
    await expect(res.json()).resolves.toMatchObject({
      ok: true,
      runtime_updates: [[1, 60, 1_776_230_000, 1_776_230_280, 'up', 'up', 21]],
      processed_count: 1,
  ...
  it('still writes fragments per batch when fragment-only mode wins over the defer header', async () => {
    const now = new Date('2026-04-15T05:18:20.000Z').valueOf();
    vi.spyOn(Date, 'now').mockReturnValue(now);
    vi.mocked(runExclusivePersistedMonitorBatch).mockResolvedValue({
      runtimeUpdates: [
